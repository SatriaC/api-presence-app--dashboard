...
@section('content')
    ...
    <div class="page-header">
        <div>
            <h2 class="main-content-title tx-24 mg-b-5">Edit Data Lokasi</h2>
            <ol class="breadcrumb">
    ...
    <div class="row row-sm">
        <div class="col-sm-12 col-md-12 col-lg-12">
            <div class="card custom-card">
                ...
                <div class="card-body">
                    <form action="{{ route('lokasi.update', $item->id) }}" method="post" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="col-12">
                        ...
                        <div class="col-12 mt-2">
                            ...
                            <select class="form-control select2" name="id_wilayah">
                                ...
                                @foreach ($wilayah as $value)
                                    ...
                                    {{$item->id_wilayah==$value->id ? 'selected' : ''}}>
                                    {{$value->nama}}</option>
                                @endforeach
                            </select>
                        </div>
                        {{-- <div class="col-12 mt-2">
                            <label for="flag">Status</label>
                            <select class="form-control" name="flag" required>
                                <option label="">Choose Status</option>
                                <option value="1" {{ $item->flag == '1' ? 'selected' : '' }}>Aktif</option>
                                <option value="2" {{ $item->flag == '2' ? 'selected' : '' }}>Tidak Aktif
                                </option>
                            </select>
                            @error('flag')
                                <div class="invalid-feedback">{{ $message }}</div>
